...
/*
 * Prepares a DNS label of service.protocol.realm using a caller-supplied fixed
 * buffer.  Since the protocol is not relevant to all callers it can be set to
 * NULL to be excluded from the label.
 */
...
/* Insert into the list, ordering by priority.  Weight is not currently
 * used. */
static void
place_srv_entry(struct srv_dns_entry **head, struct srv_dns_entry *new)
{
    ...
    /*
     * Insert an entry into this spot if the next entry has a higher priority
     * (lower priorities are preferred), or if there is no next entry (we're at
     * the end).
     */
    for (entry = *head; entry != NULL; entry = entry->next) {
        if ((entry->next != NULL &&
             entry->next->priority > new->priority) ||
            entry->next == NULL) {
            new->next = entry->next;
            entry->next = new;
            break;
                  ...
                  struct srv_dns_entry **answers)
{
    ...
    *answers = NULL;
    ...
    for (;;) {
        ...
        uri->weight = weight;
        /* Do rdlen - 4 due to the priority and weight. */
