 ...
 * the transport, master status, and host.  The host is either an address or
 * hostname with an optional port, or an HTTPS URL.
 * The format is krb5srv:[M]:udp|tcp|kkdcp:host
 *
 * Return a NULL *host_out if there's any problems parsing the URI.
 */
static void
...

{
    ...
    *master_out = -1;
    ...
    if (uri == NULL || (strncasecmp(uri, "krb5srv", 7) != 0))
        return;
    ...
    /* Look for a master flag or an empty flags field. */
    if (*uri == 'm' || *uri == 'M') {
        master = TRUE;
        if (*++uri == '\0' || *uri != ':')
            return;
    ...
    /* The rest of the URI is the host and any optional parts. */
    *host_out = uri + 1;
